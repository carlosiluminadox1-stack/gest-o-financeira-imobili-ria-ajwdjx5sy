...
export const ConverterEmVendaModal: React.FC<ConverterEmVendaModalProps> = ({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-[#121722] border-[#232A3B] text-slate-100 max-w-2xl max-h-[92vh] overflow-y-auto p-5 sm:p-6 shadow-2xl">
        <DialogHeader className="pb-3 border-b border-[#232A3B]/60">
        ...
        {loadingInitial ? (
          ...
          <form onSubmit={handleConfirmarConversao} className="space-y-4 pt-2">
            {/* Bloco de Contexto da Transação Original */}
            <div className="p-3 rounded-xl bg-[#0B0E14] border border-[#232A3B] text-xs space-y-1">
              <div className="flex items-center justify-between text-slate-400">
                <span className="font-semibold text-slate-300">Transação Bruta do Extrato:</span>
                <span className="font-bold text-emerald-400">{formatCurrency(totalTransacao)}</span>
              </div>
              <p className="text-slate-300 truncate" title={transacao.descricao}>
                {transacao.descricao}
              </p>
              <div className="text-[10px] text-slate-500 flex items-center gap-3 pt-0.5">
                <span>Data: {new Date(transacao.data).toLocaleDateString('pt-BR')}</span>
                {transacao.observacoes && (
                  <span className="truncate max-w-[280px]">Obs: {transacao.observacoes}</span>
                )}
              </div>
            </div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => updateCurrentPart({ pctCorretor: Number(e.target.value) })}
                        className="bg-[#0B0E14] border-[#232A3B] text-slate-100 font-bold text-xs h-10 text-center"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => updateCurrentPart({ pctCaptador: Number(e.target.value) })}
                        className="bg-[#0B0E14] border-[#232A3B] text-slate-100 font-bold text-xs h-10 text-center"
                      />
                    </div>
                  </div>
                  ...
                  {currentPart.showSecondCorretor && (
                    <div className="pt-2 border-t border-[#232A3B]/50 flex items-center gap-2">
                      <select
                        ...
                        onChange={(e) => updateCurrentPart({ corretorSecundario: e.target.value })}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => updateCurrentPart({ status: e.target.value as VendaStatus })}
                      className="w-full bg-[#0B0E14] border border-[#232A3B] text-slate-100 text-xs rounded-lg h-9 px-2.5 outline-none focus:border-[#E63946]"
                    >
            ...
            <DialogFooter className="pt-3 border-t border-[#232A3B]/60 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
